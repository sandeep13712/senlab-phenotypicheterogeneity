...
class CellMigration2DSteppable(SteppableBasePy):    
    ...
    def start(self):
        for cell in self.cellListByType(1):                                
            tempVar = np.random.normal(129.375,21.525,1)
            fileName='InitialVolume.csv' 
            cell.targetVolume=tempVar[0]
            if cell.targetVolume<60:
                cell.targetVolume=60
            if cell.targetVolume>210:
                cell.targetVolume=210
            cell.targetSurface = 3.54*sqrt(cell.targetVolume)
            cell.lambdaSurface = 0.5
            tempVar1 = np.random.normal(1.00256,0.12041,1)
            cell.lambdaVolume = tempVar1[0]
            try:
                fileHandle,fullFileName=self.openFileInSimulationOutputDirectory(fileName,"a")
            except IOError:
                print "Could not open file ", fileName," for writing. "
                return
            print >>fileHandle,cell.targetVolume,",",cell.lambdaVolume 
    def step(self,mcs):
        pass
    def finish(self):
...
#                     fileHandle.close()
            ...
            print >>fileHandle,mcs,",",cell.xCOM,",",cell.yCOM,",",cell.volume,",",cell.surface
...
class ECMDegradationSteppable(SteppableBasePy):
	...
	totalFibersDeleted = 0
        ...
        for x,y,z in self.everyPixel():
             ...
             if(cell1 and cell1.type==2 and fieldMMP[x,y,z]>=1):
...
class IdFieldVisualizationSteppable(SteppableBasePy):
    def __init__(self,_simulator,_frequency=10):
        ...
        for cell in self.cellListByType(1):
            ...
            self.scalarCLField[cell]=((cell.targetVolume*0.0067)-0.4)
